<?php 
session_start();
 $_SESSION['url'] = 'vistas/Mantenimiento_Tipo_Produccion.php';
 ...
 if(isset($_SESSION['usuario'])) {
     ...
     $sqlPermisos = "SELECT * FROM permisos WHERE Id_rol = $idRolUsuario AND id_objeto = 45";
...
<div class="containertable">
    <div class="d-flex justify-content-between align-items-end mb-4">
        <div>
            <h1 class="poppins-font mb-2">TIPOS DE PRODUCCION</h1>
            <br>
...
<table id="tablax" class="table table-hover">
            ...
            <tbody class="text-center">
                <?php
                include "../php/conexion_be.php";
                $sql = $conexion->query("SELECT * FROM tbl_tipo_produccion");
                while ($datos = $sql->fetch_object()) { ?>
                    <tr>
                        <td><?= $datos->id_tipo_produccion ?></td>
                        ...
                        <?php if ($permiso_actualizacion == 1) : ?>
                            ...
                            ('<?= $datos->id_tipo_produccion ?>', '<?= $datos->tipo_produccion ?>', '<?= $datos->descripcion ?>', '<?= $datos->estado ?>')">
                                <i class="bi bi-pencil-square"></i>
                                Editar
                            </button>
                            <?php endif; ?>
 				<?php if ($permiso_eliminacion == 1) : ?>
                            <form id="deleteForm" method="POST" action="modelos/eliminar_tipo_produccion.php" style="display: inline;">
                                <input type="hidden" name="id_tipo_produccion" value="<?= $datos->id_tipo_produccion ?>">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header" style="background-color: #17A2B8;">
                <h5 class="poppins-modal mb-2" id="exampleModalLabel">EDITAR PRODUCCION</h5>
                    ...
                    <div class="row">
                        <div class="col-6">
                            <div class="form-group">
                                <label for="id_tipo_produccion">Código</label>
                                <input type="text" class="form-control" id="id_tipo_produccion" name="id_tipo_produccion" readonly>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="form-group">
...
<!-- Modal para crear produccion -->
<div class="modal fade" id="modalForm" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content" role="document">
            <div class="modal-header" style="background-color: #17A2B8;">
                <h5 class="poppins-modal mb-2" id="exampleModalLabel">CREAR TIPO PRODUCCION</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Cerrar">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="modelos/agregar_tipo_produccion.php" method="POST">
                    <div class="row mb-3">
                        <div class="col">
                            <label for="tipo_produccion" class="form-label">Tipo de producción</label>
                            <input type="text" class="form-control" id="tipo_produccion" name="tipo_produccion">
                        </div>
                        <div class="col">
...
<!-- JavaScript para manejar la edición de produccion -->
<script>
    // Función para abrir el modal de edición
    function abrirModalEditar(id_tipo_produccion, tipo_produccion, descripcion, estado) {
        document.getElementById("id_tipo_produccion").value = id_tipo_produccion;
        document.getElementById("tipo_produccion").value = tipo_produccion;
...
<!-- Script para mostrar el mensaje al momento de editar una produccion-->
<script>
    $(document).ready(function() {
        $("#formularioEditar").on("submit", function(event) {
            ...
            $.ajax({
                url: "modelos/editar_tipo_produccion.php",
                method: "POST",
                data: $(this).serialize(),
                success: function(response) {
                    if (response == "success") {
                        Swal.fire({
                            title: "Tipo de producción actualizada correctamente",
                            text: "El tipo de producción se ha actualizado correctamente.",
                            icon: "success",
                        ...
                        Swal.fire({
                            title: "Error",
                            text: "Ya existe un tipo de produccion con el mismo nombre.",
                            icon: "error",
                            confirmButtonText: "Cerrar"
                        }).then(function() {
                            location.reload(); // Recarga la página
                        ...
                        Swal.fire({
                            title: "Error",
                            text: "Hubo un problema al actualizar el Tipo de produccion.",
                            icon: "error",
                            confirmButtonText: "Cerrar"
                        }).then(function() {
                            location.reload(); // Recarga la página
...
<script>
    $(document).ready(function() {
        $("form#deleteForm").on("submit", function(event) {
            ...
            Swal.fire({
                title: "¿Estás seguro?",
                text: "Esta acción eliminará el tipo de producción. Esta acción no se puede deshacer.",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response == "success") {
                                Swal.fire({
                                    title: "Tipo de producción eliminada correctamente",
                                    text: "Tipo de produccioón se ha eliminado correctamente.",
                                    icon: "success",
                                    showCancelButton: false,
                                    confirmButtonText: "Cerrar"
                                }).then(function() {
                                ...
                                Swal.fire({
                                    title: "Error",
                                    text: "Hubo un problema al eliminar el tipo de producción.",
                                    icon: "error",
                                    confirmButtonText: "Cerrar"
                                }).then(function() {
                                    location.reload(); // Recarga la página
