<?php
session_start();
$_SESSION['url'] = 'vistas/Mantenimiento_Relevo_Organizacion.php';
...
<div class="containertable">
    <div class="d-flex justify-content-between align-items-end mb-4">
        <div>
            <h1 class="poppins-font mb-2">Relevo Generacional</h1>
            <br>
        ...
        <div class="mb-4 border-bottom">
            <form class="d-flex" role="search">
                <div class="input-group">
                    ...
                    <input class="form-control" id="searchInput" type="search" placeholder="Buscar Relevo..." aria-label="Search">
                </div>
            </form>
        </div>
    </div>
        ...
        <table class="table table-hover">
            <thead class="table-dark text-center" style="background-color: #343A40;">
                <tr>
                    ...
                    <th scope="col">Tendra Relevo</th>
                    <th scope="col">cuantos Relevos</th>
            ...
            <tbody class="text-center">
                ...
                $sql = $conexion->query("SELECT r.Id_Relevo, r.id_ficha, r.id_productor ,p.primer_nombre, r.tendra_relevo, r.cuantos_relevos, r.descripcion, r.estado FROM tbl_relevo_organizacion r INNER JOIN tbl_productor p ON r.id_productor = p.id_productor;");
                while ($datos = $sql->fetch_object()) { ?>
                    <tr>
                        <td><?= $datos->id_ficha ?></td>
                        <td><?= $datos->primer_nombre ?></td>
                        <td><?php
                            if ($datos->tendra_relevo == "S") {
                                echo '<h6><span class="badge bg-info">SI</span></h6>';
                            } else {
                                echo '<span class="badge bg-danger">NO</span>';
                            }
                            ?></td>
                        <td><?= $datos->cuantos_relevos ?></td>
                        ...
                        <td>
                            ...
                            ('<?= $datos->Id_Relevo ?>', '<?= $datos->id_ficha ?>', '<?= $datos->id_productor ?>', '<?= $datos->tendra_relevo ?>','<?= $datos->cuantos_relevos ?>','<?= $datos->descripcion ?>','<?= $datos->estado ?>')">
                                <i class="bi bi-pencil-square"></i>
                                Editar
                            </button>
                            <form id="deleteForm" method="POST" action="modelos/eliminar_periodicidad.php" style="display: inline;">
                                <input type="hidden" name="Id_Relevo" value="<?= $datos->Id_Relevo ?>">
            ...
            <div class="modal-body">
                <form id="formularioEditar" method="POST" action="#">
                    <div class="row">
                        ...
                        <div class="col">
                            <label class="form-label">¿Tiene relevos?</label>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="tendra_relevo" id="tendra_relevo_S" value="S">
                                <label class="form-check-label" for="tendra_relevo_S">Sí</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="tendra_relevo" id="tendra_relevo_N" value="N">
                                <label class="form-check-label" for="tendra_relevo_N">No</label>
                            </div>
                        </div>
                        <div class="col-6">
                        ...
                        <div class="col-6">
                            <label for="cuantos_relevos" class="form-label">¿Cuantos Relevos?</label>
                            <input type="number" class="form-control" id="cuantos_relevos" name="cuantos_relevos" pattern="[0-9]*" inputmode="numeric">
                        </div>
                        <div class="col-6">
                        ...
                        <div class="col-6">
                            <div class="form-group" style="display: none;">
                                <label for="Id_Relevo"> Id_Relevo</label>
                                <input type="text" class="form-control" id="Id_Relevo" name="Id_Relevo" readonly>
                            </div>
                        </div>
                    </div>
...
<div class="modal fade" id="modalForm" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content" role="document">
            ...
            <div class="modal-body">
                <form action="modelos/agregar_relevo_organizacion.php" method="POST">
                    <div class="row mb-3">
                        <div class="col-6">
                            <div class="form-group">
                                <label for="id_ficha">Ficha:</label>
                        ...
                        <div class="col">
                            <label class="form-label">¿Tiene relevos?</label>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="tendra_relevo" id="tendra_relevo_si" value="S">
                                <label class="form-check-label" for="tendra_relevo_si">Sí</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="tendra_relevo" id="tendra_relevo_no" value="N">
                                <label class="form-check-label" for="tendra_relevo_no">No</label>
                            </div>
                        </div>
                        <div class="col-6">
                            <label for="cuantos_relevos" class="form-label">¿Cuantos Relevos?</label>
...
<script>
    ...
    function abrirModalEditar(Id_Relevo, id_ficha, id_productor, tendra_relevo, cuantos_relevos, descripcion, estado) {
    document.getElementById("Id_Relevo").value = Id_Relevo;
    document.getElementById("id_ficha").value = id_ficha;
    document.getElementById("id_productor").value = id_productor;
    // Marcar el radio button correspondiente
    if (tendra_relevo === 'S') {
        document.getElementById("tendra_relevo_S").checked = true;
    } else if (tendra_relevo === 'N') {
        document.getElementById("tendra_relevo_N").checked = true;
    }
    document.getElementById("cuantos_relevos").value = cuantos_relevos;
...
<script>
    $(document).ready(function() {
        $("#formularioEditar").on("submit", function(event) {
            ...
            $.ajax({
                url: "modelos/editar_relevo_organizacion.php",
                method: "POST",
                data: $(this).serialize(),
