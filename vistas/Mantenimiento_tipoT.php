    <?php 
    session_start();
    $_SESSION['url'] = 'vistas/Mantenimiento_tipoT.php';
    ...
    if(isset($_SESSION['usuario'])) {
        ...
        $sqlPermisos = "SELECT * FROM permisos WHERE Id_rol = $idRolUsuario AND id_objeto = 51";
 ...
 <!-- <link rel="stylesheet" href="https://cdn.datatables.net/1.10.20/css/jquery.dataTables.min.css"> -->
    ...
    <div class="containertable">
        <div class="d-flex justify-content-between align-items-end mb-4">
            <div>
                <h1 class="poppins-font mb-2">TIPO DE TRABAJADORES</h1>
                <br>
    ...
    <table id="tablax" class="table table-hover">
                ...
                <tbody class="text-center">
                    <?php
                    include "../php/conexion_be.php";
                    $sql = $conexion->query("SELECT * FROM tbl_tipo_trabajadores");
                    while ($datos = $sql->fetch_object()) { ?>
                        <tr>
                            <td><?= $datos->id_tipo_trabajador  ?></td>
                            ...
                            <?php if ($permiso_actualizacion == 1) : ?>
                                ...
                                ('<?= $datos->id_tipo_trabajador ?>', '<?= $datos->tipo_trabajador ?>', '<?= $datos->descripcion ?>', '<?= $datos->estado ?>')">
                                    <i class="bi bi-pencil-square"></i>
                                    Editar
                                </button>
                                <?php endif; ?>
 				<?php if ($permiso_eliminacion == 1) : ?>
                                <form id="deleteForm" method="POST" action="modelos/eliminar_tipoT.php" style="display: inline;">
                                    <input type="hidden" name="id_tipo_trabajador" value="<?= $datos->id_tipo_trabajador ?>">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header" style="background-color: #17A2B8;">
                    <h5 class="poppins-modal mb-2" id="exampleModalLabel">EDITAR TIPO DE TRABAJADOR</h5>
                        ...
                        <div class="row">
                            <div class="col-6">
                                <div class="form-group">
                                    <label for="id_tipo_trabajador">Código</label>
                                    <input type="text" class="form-control" id="id_tipo_trabajador" name="id_tipo_trabajador" readonly>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="form-group">
                                    <label for="tipo_trabajador">Tipo de trabajador </label>
                                    <input type="text" class="form-control" id="tipo_trabajador" name="tipo_trabajador" required>
                                </div>
                            </div>
                        </div>
                        <div class="row">
    ...
    <div class="modal fade" id="modalForm" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content" role="document">
                <div class="modal-header" style="background-color: #17A2B8;">
                    <h5 class="poppins-modal mb-2" id="exampleModalLabel">CREAR TIPO DE TRABAJADOR </h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Cerrar">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="modelos/agregar_tipoT.php" method="POST">
                        <div class="row mb-3">
                            <div class="col">
                                <label for="tipo_trabajador" class="form-label">Tipo de trabajador</label>
                                <input type="text" class="form-control" id="tipo_trabajador" name="tipo_trabajador">
                            </div>
                            <div class="col">
    ...
    <script>
        ...
        function abrirModalEditar(id_tipo_trabajador, tipo_trabajador, descripcion, estado) {
            document.getElementById("id_tipo_trabajador").value = id_tipo_trabajador;
            document.getElementById("tipo_trabajador").value = tipo_trabajador;
    ...
    <script>
        $(document).ready(function() {
            $("#formularioEditar").on("submit", function(event) {
                ...
                $.ajax({
                    url: "modelos/editar_tipoT.php",
                    method: "POST",
                    data: $(this).serialize(),
                    success: function(response) {
                        if (response == "success") {
                            Swal.fire({
                                title: "Tipo de trabajador actualizado correctamente",
                        ...
                        Swal.fire({
                            ...
                            Swal.fire({
                                title: "Error",
                                text: "Hubo un problema al actualizar el tipo de trabajador.",
                                icon: "error",
                                confirmButtonText: "Cerrar"
                            }).then(function() {
                                location.reload(); // Recarga la página
    ...
    <script>
        $(document).ready(function() {
            $("form#deleteForm").on("submit", function(event) {
                ...
                Swal.fire({
                    title: "¿Estás seguro?",
                    text: "Esta acción eliminará el tipo de trabajador. Esta acción no se puede deshacer.",
                    icon: "warning",
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response == "success") {
                                    Swal.fire({
                                        title: "Tipo de trabajador eliminado correctamente",
                                        text: "El tipo de trabajador se ha eliminado correctamente.",
                                        icon: "success",
                                        showCancelButton: false,
                                        confirmButtonText: "Cerrar"
                                    }).then(function() {
                                    ...
                                    Swal.fire({
                                        title: "Error",
                                        text: "Hubo un problema al eliminar el tipo de trabajador.",
                                        icon: "error",
                                        confirmButtonText: "Cerrar"
                                    }).then(function() {
                                        location.reload(); // Recarga la página
